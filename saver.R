...
for(pb in c(0.01,0.05,0.1)){
  for(rp in 1:6){
    ...
    mdc.saver <- saver(mdc_mask$mt,ncores = 3)
    fn1 <- paste("./result/saver",pb,rp,"/",sep = "-")
    dir.create(fn1)
    fn2 <- paste("./result/saver",pb,rp,"/data.RD",sep = "-")
    save(ind_gene_5k,mdc_mask,mdc.saver,file = fn2)
  }
}
